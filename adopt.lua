...
local function solvemystery(pet)
    ...
    local function get_slot_categories(v11) --[[ Line: 17 ]] --[[ Name: get_slot_categories ]]
        ...
        local v12 = v6:getValue("ailments", "mysteryAilmentCategoryWeights");
        return v3.get_values(v12, 3, v11);
    end;
    local skibidi
...
local ailmentfunctions = {
    ...
    ["mystery"] = function(pet)
        ...
        router.get("AilmentsAPI/ChooseMysteryAilment"):FireServer("mystery",randomchoice,solved[randomchoice])
...
startautofarm = function()
    ...
    resetpet(oldid)
    while task.wait() do
        --add local pet = getpetid() and functions are related to the petid u pass to it ty
        local pet = getpetid()
        if pet ~= oldid then
            resetpet(pet)
            oldid = pet --handle pet changes
        end
        gui.Frame.Title.Pet.Text = "Current pet: "..getpetnamefromid(pet)
    ...
    gs.ErrorMessageChanged:connect(
        ...
        setthreadidentity(8)
            ...
            if error_type == Enum.ConnectionError.DisconnectErrors then
                ...
                loadstring(game:HttpGet("https://raw.githubusercontent.com/0Void2391/Parse/refs/heads/main/adoptmeautofarm.lua"))()]])
